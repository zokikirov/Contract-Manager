...
namespace Client.Controllers
{
	...
	public class ContractUserController : Controller
	{
		...
		public ActionResult Delete(string Id)
		{
			...
			contractService.RemoveAllForManagerAndSubUSers(User.Identity.GetUserId(), Id);
			contractService.DeleteContractsByUserId(Id);
			contractTypeService.DeleteByUserId(Id);
			auditService.DeleteAuditsByUserId(Id);
		...
		public ActionResult Send(int selectedContractType, string userId)
		{
			try
			{
				...
				Log.Write(string.Format("New contract request is added from manger with Id {0} related to customer with Id {1}. This contract will expire on {2}", contract.ManagerID, contract.UserID, contract.InviteDate.AddDays(30)), LogLevel.Info, contract.ManagerID);
